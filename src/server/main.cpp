...
Config load_config(const std::string &path) {
    ...
    cfg.zmq_port = j.value("zmq_port", 5555);
    cfg.http_port = j.value("http_port", 8080);
    cfg.local_cluster_name = j.value("local_cluster_name", "");
    cfg.local_cluster_id = j.value("local_cluster_id", (uint16_t)0);
  ...
  std::thread zmq_thread([&]() {
      try {
          ...
          while (true) {
              ...
              size_t data_idx = 1;
              if (recv_msgs[data_idx].empty()) data_idx++;

              uint32_t principal_id = 0;
              if (recv_msgs[data_idx].size() == 4) {
                  std::memcpy(&principal_id, recv_msgs[data_idx].data(), 4);
              }
              data_idx++;

              std::string opcode = recv_msgs[data_idx].to_string();
              data_idx++;
